...
4. 想随时看自己的数据：
   ...
   - `/inspect` 或 `/chakan`（省略玩家名=自己）打印科技瓶经验。
   - `/life` 显示距离下次自动跃迁剩余时间。
...
> 验证：出生点 13 个原版 market，每个只卖一种货币；**货物产出固定 normal**，价格按物品 q 品质货币
> （epic 买大需求散件、legendary 买设备/插件）。市场铺混凝土地坪、整齐对齐、有地图钉子。

1. 先备好货币：跑一遍 B、C 的灌数据 + `/reset`（等 1 秒放置）。复活后身上有 epic/legendary 瓶子（货币一）+ 普通金币（在线时长，货币二）。
2. 出生点**北面**（-Y 方向，看地图钉子）应看到 **13 个市场**：12 个科技瓶市场 3 列 × 4 行，金币市场最上方。布局（相对出生点的格偏移，北=负 Y，间距 5 格）：

   ```
                       [金币] (0,-26)
        [自动化] [物流] [军事]        y=-21   x = -5, 0, 5
        [化工]   [生产] [通用]        y=-16
        [太空]   [冶金] [电磁]        y=-11
        [农业]   [低温] [普罗米修斯]  y=-6
                  ★出生点(0,0)
   ```
   （顺序 = `constants.science_packs`；每个市场铺 5×5 混凝土，连成整片广场。）
3. 走到 **自动化市场**，按 **E** 打开 → 原版交易界面，列出物品 offer（每件一条）。
4. **验证定价/产出**（关键）：用 **epic 红瓶**买散件（带/管/杆）→ 得 **normal 品质、数量多**（如 ×50）；用 **legendary 红瓶**买设备（机器/插件）→ 得 **normal 品质、数量少**（如 ×1）。产出一律 normal，价格分别要 epic / legendary 瓶。
...
| `/inspect <名字>` | 所有人 | 查看某玩家的科技瓶经验（省略=自己；无经验时静默） |
| `/chakan <名字>` | 所有人 | 同 `/inspect`，中文拼音别名 |
