...
- **可调常量**（默认值由 `constants.ensure_defaults` 设，on_init / on_configuration_changed / 每轮 reset 都调用，幂等不覆盖；游戏内 `/c storage.xxx=N` 动态调）：
  ...
  - 每星每外星【前往】概率 `travel_chance[星球]`(默认1.0) / 投票+前往共享冷却 `action_cd_minutes`(默认3) / 每事件独立周期 `event_period_min[et]`(默认1分钟)
  ...
  - `storage.event_types.<raid/meteor/supply/coinfall/drones/barrage/tech/thunder>`：事件世界各类型开关（初始默认见 `event_defaults`：raid/meteor/coinfall 默认关），`/c` 设 false 即排除某类型
