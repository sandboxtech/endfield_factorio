...
2. **在线时长** → 人物等级（= 开局金币 `floor(√在线分钟)`）、星星（在线奖励）。

（手搓/移动/采矿等只作为角色面板的统计数据展示，不再提供任何速度加成。）
...
| `passives.lua` | **行为统计累积**：手搓秒数/移动格数/采矿次数/击杀/毁巢，仅供角色面板统计展示，不再驱动任何加成。独占 craft/mine/changed_position 事件 + died(经总线)。 |
...
受多种瓶子加成的【双瓶职业】（各按对应瓶各发一种后者）：后勤官 主动供应箱 /（物流→物流机器人、通用→建设机器人）、军工专家 激光炮塔 /（军事→火炮、生产→蜘蛛车）、能源大师 变电站 /（化学→核反应堆、电磁→蓄电池）、拓荒者 加热塔 /（农业→生化室、低温→聚变反应堆）。
