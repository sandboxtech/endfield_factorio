...
- `storage.platform_age[idx]` / `platform_lifetime`：飞船经历跃迁数 / 上限。
...
on_surface_cleared（每星球，clear 结算后）:
    ...
    滚世界变体: ground_tint / tile_remap / danger_theme / event_world / loot_style（debug 打印给管理员）
    母星: market.place_on_nauvis()

on_chunk_generated（每区块）:
    圆外铺虚空 -> map_features.generate（异物/树调色/战利品箱/危险敌群/残骸）
