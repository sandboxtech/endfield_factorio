...
function M.ensure_defaults()
    ...
    local d = {
        ...
        max_platform_size = 512,          -- 飞船(平台)最大尺寸：新建平台表面的 map_gen_settings 宽/高都钳到此值 → 最大可建面积 size×size。0/nil=不限
        max_platform_count = 30,          -- 飞船(平台)最大数量：玩家阵营平台总数超过此值时，新成形的平台被销毁并广播。0/nil=不限
