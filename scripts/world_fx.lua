...
-- 复制虫：玩家建筑被【虫】破坏时，按【本轮危险度 danger】概率原地冒出新虫 → 防御被打穿会滚雪球。
...
register('replicant', defines.events.on_entity_died, function(e)
    ...
    if surface.platform then return end   -- 太空平台不刷虫（小行星等敌方死因不该在平台上冒虫）
    local mul = storage.replicant_mul or 1   -- 概率乘数常数（0=关；老存档无此键按 1 兜底）
    if mul <= 0 then return end
    if math.random() >= map_features.knobs().danger * mul then return end   -- 概率 = 本轮危险度 danger × 乘数（危险世界爆虫多、易滚雪球）
