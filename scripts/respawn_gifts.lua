...
--   ② 主格物品清单（M.gift_list）= 起手基础物资 + 开局金币 + 【职业】前者(无条件1组) + 后者(每100级1组)。
...
-- 经验 = 累计瓶数×品质系数（每个瓶 1 点，见 constants.bottles_per_exp=1）；满级(10000 级)需该瓶累计 1×10^8 经验。
M.MAX_LEVEL = 10000
...
-- 本轮首次复活【发到背包主格】的完整物品清单：起手基础物资 + 金币 + 职业前者(无条件1组) + 后者(每100级1组)。
